...
def test_api_can_reopen_persisted_incident_and_audit(tmp_path: Path) -> None:
    ...
    authenticator = BearerTokenAuthenticator(
        token="persistent-alert-token", subject="test-alertmanager"
    )
    client = TestClient(
        create_app(database_url=database_url, alert_authenticator=authenticator)
    )
    client.headers.update({"Authorization": "Bearer persistent-alert-token"})
    payload = {
        "status": "firing",
        "alerts": [
            {
